...
for iter = 1:maxiter
    ...
    [xi,Atxi,proju,x,Ax,itSN] = SNCG_DALMQ(xi,Atxi,Amap,ATmap,b,OPTIONS_SNDir,x,lambda,sigma,mu);
 ...
 %***********************  update the Lagrange multiplier *******************
    ...
    pobj = 0.5*norm(As)^2 - b'*xvar + lambda*sum(abs(xvar))+(mu/2)*norm(xvar)^2;
...
% *************************** stopping condition ***************************
    ...
    if info.rhoflag == 1
        ...
        prox_res = xvar - Prox_L1(xvar+b-Gxvar,lambda);
    ...
    if (printyes) &&(mod(iter,2)==0)
        
        fprintf('\n %3d    %3d       %3.5e       %3.5e      %3.2f  %3.2f',iter,itSN,resi_in,distq,sigma,ttime);
    end
 ...
 %% *************** update the parameters and variables **************
    ...
    if mod(iter,sigma_update_iter) ==0
        if (prim_win>max(1,dual_win))
            prim_win = 0.0;
            sigma = min(1e+4,sigma*rac);
            
        elseif (dual_win>max(1,prim_win))
            dual_win = 0.0;
            sigma = max(1.0e-4,sigma/rac);
        end
   end 
